...
public class Mail {
    ...
    public String sendMail() throws Exception {
       ...
       /*// 1. 创建参数配置, 用于连接邮件服务器的参数配置
        ...
        // 3. 创建一封邮件
        MimeMessage message = createMimeMessage(session, mailInfo);
        ...
        transport.connect(mailInfo.getFromMail(), mailInfo.getFrompassword());
     ...
     * 创建一封复杂邮件（文本+图片+附件）
     */
    private MimeMessage createMimeMessage(Session session, MailInfo mail) throws Exception {
        // 1. 创建邮件对象
        MimeMessage message = new MimeMessage(session);

        // 2. From: 发件人
        message.setFrom(new InternetAddress(mail.getFromMail(), mail.getFromName(), "UTF-8"));
